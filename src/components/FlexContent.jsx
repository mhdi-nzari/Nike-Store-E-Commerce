...
const FlexContent = ({ifExists,  endpoint:{heading,title , subtitle , text , img , url , btn}}) => {
  return (
    <>
      <div className={`flex items-center  justify-between lg:flex-col lg:justify-center nike-container gap-16 ${ifExists ? "flex-row-reverse" : 'flex-row'}`}>
...
<button type="button" className="button-theme bg-slate-900 shadow-slate-900 text-slate-100 py-1.5">{btn}</button>
        ...
        <div className="flex items-center justify-center max-w-xl relative  lg:max-w-none w-full">
          <img src={img} alt={`image/${heading}`} className={`w-auto object-fill transition-theme  ${ifExists ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12 '}`} />
        </div>
      </div>
    </>
  );
